...
export const DELEGATES = JSON.parse(env.PINSERVICE_DELEGATES);
if (!Array.isArray(DELEGATES))
  throw new Error(
    'PINSERVICE_DELEGATES should be a JSON array with one or more delegates, e.g. ["/ip4/161.230.143.49/tcp/64885/p2p/12D3KooWCfksHSx489oMAH2ysfNTvVtzQLj4u5PHfrXckYNzUU4x"].' +
      "Consider running: PINSERVICE_DELEGATES=`ipfs id | jq -r -c '.Addresses'` npm start"
  );
